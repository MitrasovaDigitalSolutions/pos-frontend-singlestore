...
export function ItemsTable({
  ...
  const totalValue = items.reduce(
    (acc, item) => acc + item.kuantitas * item.harga_estimasi,
    0
  );

  if (items.length === 0) {
    return (
      <div className="border border-dashed border-slate-200 rounded-2xl p-8 text-center bg-white/60">
  ...
  return (
    <FormProvider {...methods}>
      <div className="space-y-3">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      {isFlashing && (
                        <span className="ml-2 text-[9px] font-bold text-emerald-600 bg-emerald-100 px-1.5 py-0.5 rounded-full">
                          ⚡ baru
                        </span>
                      )}
                    </td>
        ...
        {/* Mobile Summary Cards (< 768px) */}
        <div className="md:hidden bg-white border border-slate-100 rounded-2xl p-3.5 shadow-2xs space-y-2">
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-500 font-medium">Total Item</span>
            <span className="font-bold text-slate-800">
              {totalItems} pcs ({items.length} produk)
            </span>
          </div>
          <div className="flex justify-between items-center text-xs pt-2 border-t border-slate-100">
            <span className="text-slate-700 font-semibold">Total Nilai</span>
