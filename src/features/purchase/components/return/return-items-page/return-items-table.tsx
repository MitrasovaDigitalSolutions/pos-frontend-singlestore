...
export function ReturnItemsTable({
  ...
  const totalReturCount = (activeItems || []).reduce(
    (acc: number, i) => acc + (Number(i?.kuantitas) || 0),
    0
  );
      ...
      <div className="space-y-3">
        ...
        <div className="block md:hidden space-y-2.5">
          {items.map((item, idx) => (
            <ReturnItemMobileCard
              ...
              onRemoveItem={() => updateItem(item.temp_uid, { kuantitas: 0 })}
            />
          ))}
        </div>
              ...
              <tbody className="divide-y divide-slate-50 font-medium">
                {items.map((item, idx) => {
                  ...
                  return (
                    <tr
                      key={`${item.temp_uid || item.product_uid}-${idx}`}
                      id={`return-item-row-${item.temp_uid || item.product_uid}`}
                      ...
                      <td className="p-3">
                        {item.kuantitas > 0 && (
                          <button
                            type="button"
                            onClick={() =>
                              updateItem(item.temp_uid, { kuantitas: 0 })
                            }
                            disabled={isPending}
        ...
        <div className="md:hidden bg-white border border-slate-100 rounded-2xl p-3.5 shadow-2xs space-y-2">
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-500 font-medium">Total Barang Retur</span>
            <span className="font-bold text-slate-800">
              {totalReturCount} pcs
            </span>
          </div>
          <div className="flex justify-between items-center text-xs pt-2 border-t border-slate-100">
            <span className="text-slate-700 font-semibold">Total Nominal Retur</span>
